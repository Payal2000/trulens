...
import numpy as np
import pandas as pd
from trulens.core.run import _DIRECTION_WARNED
...
class TestCompareRuns(unittest.TestCase):
    ...
    def test_partial_overlap_warns(self):
        ...
        self.assertTrue(any("Dropped" in msg for msg in cm.output))
        self.assertEqual(diff.metrics["relevance"].n_items, 1)
    ...
    def test_unspecified_direction_warns(self):
        """When metric_directions omits a metric, a warning is emitted."""
        # Clear the warning sentinel so this test can trigger the warning.
        _DIRECTION_WARNED.discard("relevance")
        ...
        with self.assertLogs("trulens.core.run", level="WARNING") as cm:
            compare_runs(run_a, run_b, metric_directions={})

        self.assertTrue(any("No metric direction" in msg for msg in cm.output))
